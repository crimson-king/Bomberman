...
class KeyConfigState(State):
    ...
    def change_current_player(self):
        ...
        self.current_player = ((self.current_player+1) % configuration.players)
        self.items[self.selected].text = "Player <%d> keys" % (self.current_player+1)
        self.update_keys()

    def update_keys(self):
        """ updates key names for key mappings for current player """
        self.items[1].text = 'Action: ' + pygame.key.name(
            configuration.player_key_configs[self.current_player].action)
        self.items[2].text = 'Up key: ' + pygame.key.name(
            configuration.player_key_configs[self.current_player].up)
        self.items[3].text = 'Down key: ' + pygame.key.name(
            configuration.player_key_configs[self.current_player].down)
        self.items[4].text = 'Left key: ' + pygame.key.name(
            configuration.player_key_configs[self.current_player].left)
        self.items[5].text = 'Right key: ' + pygame.key.name(
            configuration.player_key_configs[self.current_player].right)

    def set_key(self, key):
        """ sets new key for key mapping for current player """
        if self.selected == 1:
            configuration.player_key_configs[self.current_player].action = key
        elif self.selected == 2:
            configuration.player_key_configs[self.current_player].up = key
        elif self.selected == 3:
            configuration.player_key_configs[self.current_player].down = key
        elif self.selected == 4:
            configuration.player_key_configs[self.current_player].left = key
        else:
            configuration.player_key_configs[self.current_player].right = key
        self.update_keys()
